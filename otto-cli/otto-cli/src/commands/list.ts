...
export default class List extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const designs = (result as any).designs || [];
      ...
      for (const design of designs) {
        const truncatedPrompt = design.prompt.length > 35
          ? design.prompt.substring(0, 35) + '...'
          : design.prompt;
      ...
      if (error.code === 'ECONNREFUSED' || error.name === 'TypeError') {
        this.error('✗ Failed to connect to canvas server. Make sure it is running on http://localhost:3000');
      } else {
